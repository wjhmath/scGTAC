...
ax=axes[0]
ax.scatter(syn["n_cells"],syn["minutes"],s=55,c="#5E7367",edgecolors="white",linewidths=0.6,zorder=4)
...
ax.plot(xs,linear(xs,*popt),color="#A3B899",linewidth=1.5,linestyle="--",zorder=2,label=f"Linear fit (R$^2$={r2:.3f})")
...
ax=axes[1]
ax.scatter(real["n_cells"],real["minutes"],s=55,c="#5E7367",edgecolors="white",linewidths=0.6,zorder=4)
popt2,_=curve_fit(power,real["n_cells"],real["minutes"],p0=[0.01,0.5])
xs2=np.linspace(real["n_cells"].min(),real["n_cells"].max(),200)
ax.plot(xs2,power(xs2,*popt2),color="#A3B899",linewidth=1.5,linestyle="--",zorder=2,label=f"Fit: t $\propto$ N$^{{{popt2[1]:.2f}}}$")
try:
